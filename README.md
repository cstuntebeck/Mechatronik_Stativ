...
```mermaid
graph TD
    A[Start: Program Start main.py] --> B[Init GUI & Modules]
    B --> C{Establish Connection}
    C -->|Success| D[Setup Phase]
    C -->|Error| E[Error Message]

    D --> D1[Manually Position Motor]
    D1 --> D2[Save Corner Points (TL, BL, BR)]
    D2 --> D3[Define Grid Size (X, Y)]
    D3 --> D4[Press Calculate -> Geometry Determined]
    
    D4 --> F[Start Measurement]
    
    F --> G[MesstafelWorker Thread Start]
    G --> H{Next Point?}
    
    H -->|Yes| I[Calculate Target Pos (Interpolation)]
    I --> J[Send Motor Command -> ESP32]
    J --> K[Wait for Arrival]
    K --> L[Send Measure Command -> CS-2000]
    L --> M[Wait for Data]
    M --> N[Receive & Save Data]
    N --> O[GUI Update (Visualization)]
    O --> H
    
    H -->|No (Done)| P[End: Export/Save Data]
```
